...
## CALL DESIGN SCRIPT ## coolant_initial_state, mdot_c, num_orifices, diameter_orifice, cd_orifice, pressure_orifice, temp_orifice
preset_chamber_ID = preset_chamber_ID / 39.37 # inches to meters
film_cooling_props = ["liquid",0.2, 10, 0.003175, 0.8, 0.3, 295] # coolant_initial_state ("liquid" or "gas"), total_mdot_coolant, num_orifices, diameter_orifice, orifice_cd, axial velocity fraction due to tangential injection, temp_orifice
#film_cooling_props = []
...
lfc = engine.design_engine() # Call the design function
...
if len(engine.film_cooling) > 1:
    print("C-star [m/s]: " + str(engine.lfc_cstar))
else: 
...
if len(engine.film_cooling) > 1:
    print("")
    print(" +++ FILM COOLING RESULTS +++")
    print("Coolant Mass Flow Percentage: " + str((engine.film_cooling[1]/(engine.mDot_tot+engine.film_cooling[1]))*100))
    print("Coolant Film Length [in]" , str((engine.film_cooling[-1][2])*39.3701))
    print("Film Coolant Entriainment Fraction: " + str(engine.film_cooling[-1][-1]))
    print("Loss of Cstar [m/s]: " + str(engine.lfc_cstar - engine.C_star))
else:
    print("Film Cooling: No Film Cooling")
print(" ")
plt.figure()
...
heat = Heatsink(engine, thickness = thickness, hotfire_time = hotfire_time, chamber_inner_diameter =  preset_chamber_ID*39.37, graphite_OD = graphite_OD, graphite_start_index = graphite_start_index, graphite_end_index = graphite_end_index, dt = dt, radial_subdivisions = radial_subdivisions, lfc=lfc)
...
# Save the hg_list_results array to a CSV file
# The delimiter=',' is what makes it a comma-separated (CSV) file.
#np.savetxt("hg_list_output.csv", hg_list, delimiter=",")

#print("Successfully saved hg_list to hg_list_output.csv")
