...
def bartz(engine, T_wg, i):
    ...
    #print(h_g, "__________________________________________________________-THIS IS THE BARTZ H_G")
...
def hg_boiling_liquid_film(engine, T_wl, T_hg, heat_flux_wall, M_wt, i, dz, lfc): #i is z
    ...
    #lfc=liquid_film_cooling()
    u_cool = lfc.u_inj_cool
    #print(u_cool, "______________________________________________________________coolant vel test in bartz.py")
    liquid_film = engine.film_cooling[-1][0] # instance of liquid_film_cooling
    radius = engine.engineProps[i][0]*.3048 # ft to m    ( radius = engine.engineProperties[i][0]*.3048)
    ...
    R = 8.314/M_wt #M_wt is in kg/mol (the conversion from cea units is already done in design.py for the appended film_props[])
    u_hg = engine.engineProps[i, 4]*np.sqrt(gamma_comb_products*T_hg*R) # m/s
    pressure = engine.engineProps[i, 8]*10**5 # bars to Pascals
    #mew_gas = ceaToSI(engine.engineProps[i,17])
    mew_gas = engine.engineProps[i,17]*0.0001 #assumes mew_gas was in millipoise and is converted to Pa*s
    #print(mew_gas, mew_gas*10000, "_____________________________________________________________mew gas test")
    pr_gas = engine.engineProps[i, 19]
    #cp_gas = ceaToSI(engine.engineProps[i, 14])
    cp_gas = engine.engineProps[i, 14]*1000 #*1000 converts from KJ/Kg*K to J/Kg*K
    #print(cp_gas, cp_gas, "______________________________________________________________cp gas test")

    # update liquid coolant properties
    saturation_temp = JetA.get_saturation_temp(pressure) # K
    h_fg = JetA.get_h_fg() # J/kgK | later should fix the get_h_fg method and add the temperature argument back in.
    temp_avg_cool = (T_wl+saturation_temp)/2 # average temperature between boiling surface & wall  ###took away T_wl from calc
    rho_v_cool = JetA.get_rho_v(T_hg, (engine.engineProps[5,8])*10**5 ) # coolant vapor density, kg/m^3
    ...
    diam_cc = engine.engineProps[i,0]*2
    k_t = 1
    h_g = lfc.get_h_g(k_t, hstar_fg, u_hg, u_cool, rho_v_cool, mew_gas, M_wt, pr_gas, cp_gas, T_hg, saturation_temp, enthalpy_flux_other, diam_cc) #last argument used to be --> liquid_film.radii(i)*2
                                    #Kt, hstar_fg, u_gas, u_cool, rho_gas, mew_gas, molecular_g, pr_gas,cp_gas,temp_gas,temp_sat_cool, enthalpy_flux_other, d_cc
    print(hstar_fg)
    return h_g
