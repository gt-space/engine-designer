...
from regenDesigner.chan_contour import get_chan_contour


## GENERAL INPUT PARAMETERS ##
# For basic engine configuration and adjustment
thrust = 2500 * 4.44822 # Thrust [lbf to N]
P_c = 300/14.5038 # 0.0689476 # Chamber pressure at injector face [psi to bar]
P_e = 10 / 14.5038 # Desired exit presure for expansion ratio [bar] (if in doubt put ambient)
con_rat = 3.0 # Contraction ratio
preset_chamber_ID = 5.75 / 39.37007874
...
cstar_eff = 0.9 #Need to actually implement this
numPTS = 250
...
adv_data = {"solve_bell": True, # Solve with a bell nozle instead of a conical one
            "div_ang": 13.5, # Conical divergence half angle [Deg]
            ...
            "theta_i": 22.5, # Angle leaving throat [deg] (Should be between 20 and 50)
            "percent_of_conical": 80} # Percent length compared to conical alternative (Should be ~80%)

## REGEN PARAMETERS ##
wall_thickness = 0.0005 # Thickness of liner (distance from inner wall to channel bottom) [m]
min_fin_width = 0.001 # Fin width at the throat [m]
min_channel_width = 0.0001 # Minimum allowable channel width [m] (determined by manufacturing capability)
...
max_fin_AR = 3.0 # Maximum fin aspect ratio (height/width)
max_channel_h = 0.01 # (m) Max channel height
#max_channel_h = 0.004

outlet_temp = 650 # (K) Max coolant outlet temp
uts_FOS_min = 1.5 # Minimum required ultimate factor of safety
max_wall_temp = 950 # (K)

## CALL ENGINE DESIGN SCRIPTS ##
engine = Engine(thrust, P_c, P_e, con_rat, L_star = L_star, MR = MR_design, adv_data = adv_data, cstar_eff = cstar_eff, numPTS = numPTS, preset_chamber_ID = preset_chamber_ID) # Create engine object
