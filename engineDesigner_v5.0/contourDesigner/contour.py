...
def get_contour(R_t, con_rat, exp_rat, L_star, adv_data, numPTS):
    ...
    print('conLeadInRadius' + str(conLeadInRadius))
    ...
    numPTS = int(np.floor(numPTS / 2))# Number of points defining each barrel and nozzle
    ...
    engineContour = np.concatenate((chBarrel,nozzleContour), axis=0) #Combine into one index
     # subtracting one because throat radius was literally inserted
    throatInd= throatInd-1
    ...
    return (engineContour, chBarrel, nozzleContour, R_tCurve, throatInd, conLeadInRadius, theta_e, throat_end_ind, throatInd_engprops,chBarrel_endInd)
