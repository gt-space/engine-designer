...
class RegenJacket:
    ...
    def __init__(self, engine, wall_t, min_fin_w, min_channel_width, var_channel_width,
                 T_ci,start_ind, min_channel_Re, max_fin_AR, max_channel_h, max_wall_temp, chamber_material_percent, throat_material_percent):
        ...
        self.max_wall_temp_arr = np.ones([self.numPTS - 1 - self.start_ind, 1])

        self.max_allowable_wall_temp = np.ones([self.numPTS - 1 - self.start_ind, 1])*max_wall_temp
        self.cond_w = 20

        self.conduction_array = np.ones([self.numPTS - 1 - self.start_ind, 1]) * 20

        throat_start_ind = round(chamber_material_percent*self.numPTS/100)
        throat_end_ind = round(throat_material_percent*self.numPTS/100)

        for x in range(len(self.conduction_array)):
            if x >= throat_start_ind and x <= throat_end_ind:
                self.conduction_array[x-1] = 375
                self.max_allowable_wall_temp[x-1] = 950

        self.ax_pos = engine.engineProps[:-(self.start_ind + 1):, 1]

        self.max_wall_temp = max_wall_temp

        self.material = np.ones([self.numPTS - 1 - self.start_ind, 1])
    ...
    def init_channel(self):
        ...
        #self.max_temp = np.zeros([self.numPTS - 1 - self.start_ind, 1])  #Max temp at station based on material
    ...
    def qout_calc(self, channel_h, channel_w, i):
        ...
        m = math.sqrt((2 * h_c) / (self.conduction_array[i-1] * fin_w))  # Fin efficiency length independent
    ...
    def height_optimization(self, channel_w, i, final=False):
        ...
        T_wc = T_wg - (q_in / (self.conduction_array[i-1] / self.wall_t) / (A_totg / length))  # Fix later to add in variable wall cooling
        ...
        if q2 < 0: #q_out - q_in < 0 @ min channel height -> Invalid design
            ...
            k_t = self.conduction_array[i-1] / self.wall_t
    ...
    def struct_optimzation(self, uts_FOS_min):
        ...
        while x < (self.numPTS - 1 - self.start_ind):
            ...
            delT = q_conv / (self.conduction_array[i-1] / self.wall_t)
            ...
            temp = self.max_allowable_wall_temp[i-1]
            self.max_wall_temp_arr[i - 1] = np.min([T_wg[0], temp[0]])
    ...
    def single_design(self, MR, num_channels, channel_w=.001, plot=False):
        ...
        if plot:
            ...
            plt.xlabel("Distance from Injector (m)")
            plt.ylabel("Heat Flow (W/m^2)")
            plt.plot(self.engine.engineProps[:-(self.start_ind + 1):, 1], self.h_c_arr)
            # plt.show()
            ...
            plt.plot(self.engine.engineProps[:-(self.start_ind + 1):, 1], self.T_aw_arr)
            # plt.show()
