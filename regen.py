...
conrat = 6
MR = 1.8 # Mixture ratio (lox to fuel)
...
def get_modeling_data(engine, profile, num_channels):
    # Display relevant data for slmodeling
...
def l_star():
    ...
    for i in range(12):
        ...
        engine = Engine(thrust, P_c, conrat, LStar = L_star, MR = MR) # Create engine object
        engine.design_engine() # Run engine design procedures
...
def regen(L_star):
    # conrat = get_conrat(thrust, P_c, True) # Run this to solve for conrat based on minimizing outlet temp (takes a while)
    engine = Engine(thrust, P_c, conrat, LStar = L_star, MR = MR) # Create engine object
    engine.design_engine() # Run engine design procedures
    jacket = regenJacket(engine) # Create jacket object
    (profile, T_co, mass, num_channels) = jacket.get_geometry() # Generate channel geometry
