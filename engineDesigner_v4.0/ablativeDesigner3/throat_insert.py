...
# IN THIS CODE, the OVERWRAP is silica phenolic insulation
...
t_C = .3*.0254 # throat_insert thickness (m)
minT= 3050
T_aw= ablative.size_throat_insert_thickness(burn_time=40,T_w=minT,t_C=t_C)[-3] # adiabatic wall temp is constant because we're looking at the throat only right now
maxT = T_aw

design_wall_thickness = .2*.0254 #m
...
for i in Temp_arr_start:
    t_OW_i = ablative.size_throat_insert_thickness(burn_time=40,T_w=i,t_C=t_C)[1]
    T_I_i = ablative.size_throat_insert_thickness(burn_time=40,T_w=i,t_C=t_C)[-2]
    h_g_i = ablative.size_throat_insert_thickness(burn_time=40, T_w=i,t_C=t_C)[3]
    T_OW_i = ablative.size_throat_insert_thickness(burn_time=40, T_w=i,t_C=t_C)[-1]
...
def size_thickness(ablative,thickness):
    ...
    T_w = Temp_arr[idx]
    (t_A, t_OW, q_conv, h_g, T_aw, T_I, T_OW) = ablative.size_throat_insert_thickness(burn_time=40, T_w=T_w,t_C=t_C)
...
T_w_design = T_w
(t_A,t_OW,q_conv,h_g,T_aw,T_I,T_OW)=ablative.size_throat_insert_thickness(burn_time=40,T_w=T_w_design,t_C=t_C)
print("")
