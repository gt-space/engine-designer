...
thrust = 1350*4.44822 # Thrust [N]
P_c = (400)*0.0689476 # Chamber pressure at injector face [bar]
P_e = (.5*14.7)*0.0689476 # Desired exit presure for expansion ratio [bar] (if in doubt put ambient)
#con_rat = 8.0 * D_t^-0.6 + 1.25 where D_t is in cm
con_rat = 5.5 #  Contraction ratio sized using correlation in code
...
## CALL DESIGN SCRIPT ##
engine = Engine(thrust, P_c, P_e, con_rat, L_star = L_star, MR = MR, adv_data = adv_data,ox = 'N2O',fuel = 'Isopropanol') # Create engine object given params
engine.design_engine() # Call the design function
engine.isentropic_comparison()

ablative = Ablative(engine, run_time=40, T_wall_i=3220, split=0)# ======================= GENERAL INPUT PARAMETERS  ============================
...
# ======================= DESIGN INPUTS  ============================
liner=ablative.CarbonEpoxy()
I=ablative.SilicaEpoxy()
OW=ablative.CarbonEpoxy()
t_A_design = .75*.0254 # ablator thickness (m) # if this if ablator was used as insulation
Max_OW_temp = 550 #K
liner_idx = 0 # uses the initial combustion chamber properties # This should be throat at some point

minT= 2700 # this is just to not have to do the entire temp regime
T_aw= ablative.size_ablator_thickness(burn_time=40,T_w=minT,t_A_design=t_A_design,wall_idx=liner_idx,liner=liner,OW=OW,I=I)[-3] # adiabatic wall temp is constant because we're looking at the throat only right now
maxT = T_aw
initial_design_OW_thickness = .125 * .0254 #m
...
for i in Temp_arr_start:
    t_OW_i = ablative.size_ablator_thickness(burn_time=40,T_w=i,t_A_design=t_A_design,wall_idx=liner_idx,liner=liner,OW=OW,I=I)[1]
    T_I_i = ablative.size_ablator_thickness(burn_time=40,T_w=i,t_A_design=t_A_design,wall_idx=liner_idx,liner=liner,OW=OW,I=I)[-2]
    h_g_i = ablative.size_ablator_thickness(burn_time=40, T_w=i,t_A_design=t_A_design,wall_idx=liner_idx,liner=liner,OW=OW,I=I)[3]
    T_OW_i = ablative.size_ablator_thickness(burn_time=40, T_w=i,t_A_design=t_A_design,wall_idx=liner_idx,liner=liner,OW=OW,I=I)[-1]
...
plt.scatter(Temp_arr,initial_design_OW_thickness/.0254*np.ones((len(Temp_arr),1)),label="Design Wall Thickness (in)",marker=".")
plt.legend()
...
def size_thickness(ablative,OW_thickness):
    ...
    for i in Temp_arr_start:
        ...
        (_, t_OW_i, _, h_g_i, _, T_I_i, T_OW_i)= ablative.size_liner_thickness1(burn_time=40, T_w=i, wall_idx=liner_idx,
                                                                                t_A_design=t_A_design, liner=liner,
                                                                                OW=OW, I=I)
