...
class Ablative:
    ...
    class Ablator:
        ...
        def __init__(self,mat,burn_time=[],t_A_design=[]):
            self.mat = mat
            self.DT=burn_time
            if t_A_design==[]:
                self.t = self.run_ablator_sizer(burn_time, [])
            else:
                self.t= t_A_design

        def run_ablator_sizer(self, burn_time, t_A_design):
            if t_A_design == []:
...
# ablative wall thickness sizing
    def size_liner_thickness1(self, burn_time,T_w, T_amb=272,wall_idx=[],t_A_design=[],liner=[],OW=[],I=[]):
    ...
    def run_liner_sizer(self,overwrap,insulator,ablator,burn_time,t_A_design=[]):
