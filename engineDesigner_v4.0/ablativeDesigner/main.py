...
con_rat = 4.514 #  Contraction ratio sized using correlation in code
...
np.savetxt('CONTOUR_PDR.csv', engine.engineContour, delimiter=',', fmt='%s')
...
DT_OW = 150  #Kelvin DT across CF # This is a reasonable temp drop I'd expect
SiPhen = ablative.SilicaPhenolic()
(t_A, t_OW, q_conv, h_g, T_aw, T_I, T_OW)=size_thickness(ablative,t_OW)
DT_liner = T_aw-T_I
(stress_OW,FOS_OW)= ablative.chamber_stress_analysis(CF,t_OW,DT_OW)
(stress_liner,FOS_liner)= ablative.chamber_stress_analysis(SiPhen,t_liner,DT_liner)
